...
A function is a procedure that assigns an element of set $Y$ to each element of set $X$.
...
Probability $P$ is a function ("probability law") that assigns a real number $\mathbb{R}$ to each element $E$ of the event space $\Sigma$ (i.e., each subset of the sample space $\Omega$):
...
P(E) \ge 0 \\
P(\Omega) = 1 \\
P(\cup^{\infty}_{i=1}E_i) = \sum^{\infty}_{i=1}P(E_i) 
$$

for disjoint sets/mutually exclusive events $E_1, E_2, \ldots$
