...
Statistics has its foundation in probability.
...
3.  Model Building: Classification and Prediction

In all cases, statistics is the science of applying the laws and rules of probability to samples, which are collections of values of a random variable or in fact a collection of random variables.
...
For example, probability $P$ is a function (a.k.a. "probability law" or "probability distribution") that assigns a real number $\mathbb{R}$ to each element $E$ of the event space $\Sigma$ (i.e., the set of all subsets of the sample space $\Omega$, i.e., the $\sigma$-algebra $\Sigma$, e.g., the "power set" $2^{\Omega}$ for a finite or countable sample space $\Omega$ or the "Borel set" for an uncountable sample space $\Omega$):
...
Probability measures how likely an event is to occur or a statement is to be true.

A **measure** $\mu$ is a function from a $\sigma$-algebra $\Sigma$ over set $\Omega$ to the real numbers $\mathbb{R}$
...
A probability measure is a measure with total measure one -- that is, $\mu(\Omega) = 1$ (Kolmogorov's axioms). A probability space is a measure space with a probability measure.
