...
gf_raster(posterior ~ zombies_cured + prop_success, data = d) |> gf_lims(x = c(0, 50), y = c(0, 0.2)) |> gf_refine(scale_fill_viridis_c())
```
...
\pi \sim \text{Uniform}(\text{min} : 0, \text{max} : 0.2)
$$
...
![](images/clipboard-378766852.png)

```{r}
temp <- c(19, 23, 20, 17, 23) # lake temp on this day in the past 5 years
```

![](images/clipboard-2889862467.png)
...
```{r}
# assuming i.i.d. temp observations
prod(like)
```
...
mu <- seq(8, 30, by = 0.5)
sigma <- seq(0.1, 10, by = 0.2)

d <- expand_grid(mu, sigma)

d <- d |>
  mutate(mu_prior = dnorm(mu, mean = 18, sd = 5),
         sigma_prior = dunif(sigma, min = 0, max = 10),
         prior = mu_prior * sigma_prior) |> 
  rowwise() |> 
  mutate(likelihood = exp(sum(dnorm(temp, mu, sigma, log = TRUE)))) |> 
  ungroup() |> 
  mutate(posterior = prior * likelihood / sum(prior * likelihood))

gf_raster(posterior ~ mu + sigma, data = d) |> gf_refine(scale_fill_viridis_c())
```
...
```{r}
sample_indices <- sample(nrow(d), size = 10000, replace = TRUE, prob = d$posterior)

head(sample_indices)
```

```{r}
# sample from d to calculate some new measures
samples <- d[sample_indices, c("mu", "sigma")]

head(samples)
```

The probability distribution of the average temperature (param mu):

```{r}
# visualize the mean temp
gf_histogram(~ mu, data = samples)
```
...
What is the probability that the actual (not the average) temperature 18C or above?

The probability distribution of the actual temperature (predicted temp):

```{r}
pred_temp <- rnorm(10000, mean = samples$mu, sd = samples$sigma)

gf_histogram(~ pred_temp)
```
...
```{r}
gf_raster(probability ~ mu + sigma, data = pars) |> gf_refine(scale_fill_viridis_c())
```

`pars` contains the dataframe representing the posterior zombie IQ distribution. The code below draws `sample_indices`: a sample of row numbers (a.k.a. *indices*) from the posterior.
...
When submitting this result to *The Journal of Zombieology*, you should we put the following sentence into the results section:

> We estimate the mean zombie IQ to be 42 (95% CI: \[35, 50\])

### Answering the question: How smart will the next zombie be?
...
`samples` is the data frame with a sample of posterior parameter estimates, and the code below simulates from a normal distribution *incorporating* all the uncertainty in the posterior estimates of the parameters mean `mu` and standard deviation `sigma`.

```{r}
pred_iq <- rnorm(10000, mean = samples$mu, sd = samples$sigma)

# visualize pred_iq
gf_histogram(~ pred_iq)
```
...
![](images/clipboard-1596283557.png)

![![](images/clipboard-1934054854.png)](images/clipboard-430645344.png)

```{r}
# brew install jags
# remotes::install_github("cran/rjags")
# remotes::install_github("mikemeredith/BEST")
library(BEST)
```
...
What would be a reasonable conclusion to draw from this analysis? There is some evidence that eating brains makes zombies *smarter*, but it's uncertain by how much.
...
What conclusion should we draw? There is weak evidence that eating brains make zombies smarter. And we should be better at screening for mutant zombies when doing experiments.

### What have you learned? What did we miss?

![](images/clipboard-4221905109.png)

![![](images/clipboard-2783938752.png)](images/clipboard-1553284924.png)

![![](images/clipboard-2754499041.png)](images/clipboard-1246267136.png)
