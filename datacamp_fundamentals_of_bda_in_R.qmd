...
cured_zombies <- map_int(1:n_zombies, \(x) runif(1, min = 0, max = 1) < prop_success)
...
```{r}
# count zombie cured
cured_zombies <- sum(cured_zombies)
...
cured_zombies = rbinom(n = n_samples, size = n_zombies, prob = prop_success)
...
```{r}
cured_zombies <- rbinom(n = 10000, size = 100, prob = 0.1) # sampling distribution
...
Because the `rbinom` function is *vectorized* the first value of `prop_success` is used to sample the first value in `cured_zombies`, the second value in `prop_success` is used for the second in `cured_zombies`, and so on. The result is that the samples in `cured_zombies` now also incorporate the uncertainty in what the underlying proportion of success could be.

```{r}
gf_density(~ prop_success)
```
...
This looks very different from the histogram of `cured_zombies` we got in the last exercise when `prop_success` was exactly 0.1. With the added uncertainty in `prop_success` the uncertainty over the number of cured zombies has also increased.
...
prior <- tibble(prop_success, cured_zombies)

prior
```

```{r}
gf_point(prop_success ~ cured_zombies, color = "skyblue", alpha = 0.1, data = prior) |> 
  gf_density_2d() +
