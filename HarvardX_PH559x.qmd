...
If there is no direct arrow from A to Y, the flow of association between A and Y is interrupted when we condition on the mediator B, even though A has a causal effect on Y.

![](images/image-1379316732.png)

### The fork (confounding)

L causes A and Y (i.e., A and Y share a common cause).

There is association between A and Y even though A has no causal effect on Y.

If there is no direct arrow from A to Y, the flow of association between A and Y is interrupted when we condition on the common cause L.

When we're using data to estimate the causal effect of A on Y, any component of the association between A and Y that is not due to the causal effect of A on Y is going to **bias** our estimate. In particular, when there is bias due to a common cause of A and Y, we say that there is bias due to **confounding**.
